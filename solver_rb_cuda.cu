                                       ...
                                       float* __restrict__ same)
{
    ...
    if (y <= n) {
        ...
        if (x < width - (1 - local_start)) {
            ...
            same[index] = (same0[index] + a * (neigh[index - width] + neigh[index] + 
                         neigh[index + local_shift] + neigh[index + width])) / c;
        }
    }
}
...
static void set_bnd(unsigned int n, boundary b, float* x)
{
    ...
    float* h_x = new float[(n + 2) * (n + 2)];
    CUDA_CHECK(cudaMemcpy(h_x, x, (n + 2) * (n + 2) * sizeof(float), cudaMemcpyDeviceToHost));
    
    h_x[IX_HOST(0, 0)] = 0.5f * (h_x[IX_HOST(1, 0)] + h_x[IX_HOST(0, 1)]);
    h_x[IX_HOST(0, n + 1)] = 0.5f * (h_x[IX_HOST(1, n + 1)] + h_x[IX_HOST(0, n)]);
    h_x[IX_HOST(n + 1, 0)] = 0.5f * (h_x[IX_HOST(n, 0)] + h_x[IX_HOST(n + 1, 1)]);
    h_x[IX_HOST(n + 1, n + 1)] = 0.5f * (h_x[IX_HOST(n, n + 1)] + h_x[IX_HOST(n + 1, n)]);
    
    CUDA_CHECK(cudaMemcpy(x, h_x, (n + 2) * (n + 2) * sizeof(float), cudaMemcpyHostToDevice));
    delete[] h_x;
}
