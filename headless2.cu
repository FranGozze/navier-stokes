...
__global__ void set_value_kernel(float* d, float* u, float* v, int idx, float force, float source, float* max_velocity, float* max_density)
{
    int gtid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gtid == 0) {
        if ((*max_velocity) < 0.0000005f) {
            v[idx] = force * 10.0f;
            u[idx] = force * 10.0f;
        }
        if ((*max_density) < 1.0f)
...
static void react(float* d, float* u, float* v)
{
    ...
    set_max_values<<<1, 1>>>(d_max_velocity2, d_max_density, blocks);
